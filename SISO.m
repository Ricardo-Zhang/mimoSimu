...
%--------------------------
SNRdB= 20:0.5:30;      %SNR range, in dB; the definition of SNR is symbol power divided by noise power
Ntrials=100;     %no. of channel realizations
...
for CQI = 15
    ...
    %     
        for nSNR=1:length(SNR)
            h = sqrt(1/2)*(randn + 1i*randn); % channel model, noiseless    %         
            ...
            ThroughPut(nSNR) = ThroughPut(nSNR)+(1-BitError)*NumInfoBits*1000/1e6;
        end;
